...
def build(root=None):
    ...
    @server.tool()
    async def mobile_session(operation: Literal["open","status","pause","resume","close","recover","action_status","burst_status","reconcile"], session_id: str | None = None, device_id: str | None = None, request_id: str | None = None, evidence_kind: Literal["postcondition_verified","not_executed"] | None = None, attestation: str | None = None) -> CallToolResult:
        """Acquire/release one device lease. Recover reconnects and observes but never clears unknown writes. Resume requires a fresh observation. action_status and burst_status read device-bound journal metadata by request_id without replaying; not_found is not proof of non-execution. reconcile closes one unknown write against a fresh observation: postcondition_verified when the action's ORIGINAL semantic postcondition now holds, not_executed only when the journal records that dispatch never started (attestation kept for audit, never sufficient alone). Anything else stays blocked; reconciliation never dispatches or replays."""
        return package(await call("session", operation=operation, session_id=session_id, device_id=device_id, request_id=request_id, evidence_kind=evidence_kind, attestation=attestation))
    @server.tool()
